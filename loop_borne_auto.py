...
while True:  # Seconde boucle infinie permettant d'utiliser la commande "break" pour arreter la transaction
    ...
    DATA_setVariable("rezalOn", bool(
        REZAL_pingServeur()))  # Ping du serveur pour s'assurer que la connection est toujours présente
    ...
    if setting.rezalOn:
        ...
        # rezalMode est à True, donc on fait la synchronisation des données de la carte avec celles de la BDD
            ...
            if argent < 0:
                afficher("APPELLER REZAL",
                         3)  # Le rezal doit regarder l'historique de la carte et vérifier que toute les caisses sont synchro
                afficher("DESYNCH BDD",
                         4)  # Affichage problème (Si ce message s'affiche pendant un gala c'est pas bon: soit la personne est un tricheur, soit une box fonctionne en mode hors ligne)
                DATA_add(setting.projet_path + 'PICONFLEX2000-LOGS/LOG_QUERRY.txt',
                         QUERRY_addLog(setting.numeroBox, setting.nomBox, "DESYNCH BDD",
                                       str(STRING_uidStrToInt(UID))))  # Ajout du message dans les logs
    ...
    if not (setting.rezalOn) or not (setting.rezalMode):
        ...
        if hashCodeType != CRYPT_hashage(config.codeGuinche):
            ...
            afficher("SYNCH RFID H TYPE", 4)  # Affichage synchronisation
            RFID_setHashCodeType(config.codeGuinche, UID)  # Ecriture RFID du Hash du codeGuinche sur la carte
            DATA_add(setting.projet_path + 'PICONFLEX2000-LOGS/LOG_QUERRY.txt', QUERRY_addCarte(
                STRING_uidStrToInt(UID)))  # Ajout de la carte dans la BDD pour une future synchronisation
            afficher("SYNCH RFID H UID", 4)  # Affichage synchronisation
            RFID_setHashUID(UID)
            afficher("SYNCH RFID ARGENT", 4)  # Affichage synchronisation
