...
## Loop ##
exec(open(projet_path+'PJT_Borne_de_recharge/config.py').read())  # Permet de se servir directement de toutes les fonctionnalités de la box quand il est lancé (Très utile pour DEV)
...
while True:  # Boucle infinie du script
    try:
        exec(open(projet_path+'PJT_Borne_de_recharge/loop.py').read())  # Execution du script se répétant jusqu'à l'arrêt du système
    except:
